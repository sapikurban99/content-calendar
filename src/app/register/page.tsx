  ...
  return (
    <div className="min-h-screen bg-[#fafafa] bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:20px_20px] flex items-center justify-center p-4">
      <div className="bg-white border-4 border-black rounded-3xl p-8 max-w-md w-full shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] text-center relative overflow-hidden">
        <div className="absolute top-0 left-0 w-32 h-32 bg-lime-300 rounded-br-full -z-10 border-b-4 border-r-4 border-black -translate-x-4 -translate-y-4"></div>
        <div className="absolute bottom-0 right-0 w-24 h-24 bg-fuchsia-400 rounded-tl-full -z-10 border-t-4 border-l-4 border-black translate-x-4 translate-y-4"></div>

        <h1 className="text-4xl font-black italic tracking-tighter mb-2 z-10 relative">JOIN THE CLUB</h1>
        <p className="font-mono text-sm font-bold text-black mb-8 uppercase tracking-widest z-10 relative">Create Your Planner</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10 text-left">
          <div className="group">
            <label className="text-[10px] font-black font-mono text-black uppercase mb-1 ml-1 block">Full Name</label>
            <input 
              type="text" 
              required 
          ...
          <div className="group">
            <label className="text-[10px] font-black font-mono text-black uppercase mb-1 ml-1 block">Password</label>
            <input 
              type="password" 
              required 
              placeholder="••••••••"
          ...
          <button 
            ...
            className="w-full bg-lime-400 text-black border-2 border-black px-4 py-4 rounded-2xl font-black text-sm tracking-widest hover:bg-lime-300 transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        >
          ...
          {isGoogleLoading ? "LOADING..." : "DAFTAR DENGAN GOOGLE"}
        </button>

        <p className="mt-8 font-mono text-xs font-bold text-black relative z-10">
          Sudah punya akun? <Link href="/login" className="text-fuchsia-600 hover:text-fuchsia-500 underline decoration-2 underline-offset-4">Masuk di sini</Link>
        </p>
      </div>
    </div>
  );
